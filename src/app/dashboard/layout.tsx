...
export default function DashboardLayout({
  ...
  const hideNav = pathname.startsWith("/items/") || pathname.startsWith("/record") || pathname.startsWith("/weight") || pathname.startsWith("/cheatday");

  return (
    <div className={`min-h-screen bg-gray-50${hideNav ? "" : " pb-24"}`}>
      <InstallPrompt />
      <div className="max-w-lg mx-auto">{children}</div>

      {!hideNav && (<nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 pb-[env(safe-area-inset-bottom)]">
